...
const SignUp = () => {
    ...
    const handleSignUp = async (e) => {
        e.preventDefault()
        try {
            const response = await fetch('http://localhost:3501/signup', {
                method: 'POST',
                headers: {
  ...
  return (
    <div className='flex items-center justify-center bg-gray-500 m-auto min-h-screen'>
        <div className=" border rounded-lg border-gray-500 bg-black shadow-2xl w-2/3 sm:w-1/4 min-h-[50%] py-10">
            <h1 className="text-3xl text-center text-white py-4">Sign Up</h1>
            <div className='flex flex-col w-3/4 mx-auto'>
                <label className="text-left text-white pr-15 pt-5 pb-2">User Name</label>
                <input 
                    type="text" 
                    value={username}
